...
- **Metered billing support** - Plans can be configured to omit quantity for usage-based pricing
...
**metered**: `boolean` - Whether the plan uses metered billing. When `true`, the quantity field is omitted from Stripe subscriptions, which is required for usage-based pricing. Default: `false`.
...
### Metered Billing Plans

For usage-based pricing, you can configure plans as metered. This omits the `quantity` field from Stripe subscriptions, which is required for metered billing:
...
stripe({
    ...
    subscription: {
        ...
        plans: [
            {
                name: "api-starter",
                priceId: "price_api_starter_metered",
                metered: true, // Enables metered billing
                limits: {
                    requests: 100000, // For display purposes only
                }
            },
            {
                name: "team-fixed",
                priceId: "price_team_fixed",
                metered: false, // Traditional seat-based billing
                limits: {
                    users: 10,
                    projects: 50,
                }
...
await client.subscription.upgrade({
    plan: "api-starter",
    // seats parameter is ignored for metered plans
